...
omega_ie = 7.292115;  % Earth rotation rate in rad/s
...
Q_matrix = zeros(8);
Q_matrix(1:3,1:3) = eye(3) * GNSS_KF_configaccel_PSD * tor_s^3 / 3;
Q_matrix(1:3,4:6) = eye(3) * GNSS_KF_configaccel_PSD * tor_s^2 / 2;
Q_matrix(4:6,1:3) = eye(3) * GNSS_KF_configaccel_PSD * tor_s^2 / 2;
Q_matrix(4:6,4:6) = eye(3) * GNSS_KF_configaccel_PSD * tor_s;
Q_matrix(7,7) = (GNSS_KF_configclock_freq_PSD * tor_s^3 / 3) +...
    GNSS_KF_configclock_phase_PSD * tor_s;
Q_matrix(7,8) = GNSS_KF_configclock_freq_PSD * tor_s^2 / 2;
Q_matrix(8,7) = GNSS_KF_configclock_freq_PSD * tor_s^2 / 2;
Q_matrix(8,8) = GNSS_KF_configclock_freq_PSD * tor_s;
...
R_matrix(1:no_meas,1:no_meas) = eye(no_meas) *...
    GNSS_KF_configpseudo_range_SD^2;
...
R_matrix((no_meas + 1):(2 * no_meas),(no_meas + 1):(2 * no_meas)) =...
    eye(no_meas) * GNSS_KF_configrange_rate_SD^2;
